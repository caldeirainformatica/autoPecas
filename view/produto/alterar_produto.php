<?php
    ...
    $listarTributacao = $conProduto->listarTributacao();
?>
      ...
      <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body ">
              <form id="form_alterar_produto"  action="../../control/ConProduto.php?acao=alterar?id=" method="post">
                  <div class="col-md-6 col-lg-6">
                      ...
                      <br>
                      <label for="subgrupo">Subgrupo:</label><select required="required" name="subgrupo"  class=" form-control"id="subgrupo" required="required">
                          <option value="">Selecione</option>
                      ...
                      <label for="volume">Volume:</label><select required="required" name="volume"  class=" form-control"id="volume" required="required">
                          <option value="">Selecione</option>
                          <?php foreach ($listaVolumes as $volumes) {?>
                            <option value="<?php echo $volumes['id_volume']?>"><?php echo $volumes['descricao']?></option>
                         <?php } ?>
                      </select> <br>
                      <label for="genero">Gênero:</label><select required="required" name="genero"  class=" form-control"id="genero" required="required">
                          <option value="">Selecione</option>
...
<!--                  <div class="form-group">-->
                <div class="col-md-6 col-lg-6">
                      ...
                      <label for="tel">Tributação:</label><select required="required" name="tributacao"  class=" form-control"id="tributacao" required="required">
                          <option value="">Selecione</option>
                          <?php foreach ($listarTributacao as $tributacao) {?>
                            <option value="<?php echo $tributacao['id_tributacao']?>"><?php echo $tributacao['descricao']?></option>
                         <?php } ?>
                      </select> <br>
                  <fieldset>
                      <legend>Observação:</legend>
                      <textarea class="form-control" name="observacao" rows="4" id="observacao" cols="50"></textarea> 
                  </fieldset>
                    <br>
